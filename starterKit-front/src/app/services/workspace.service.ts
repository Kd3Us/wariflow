...
import { WorkspaceStep, WorkspaceData, PersonForm, PersonFormData, VisualIdentityForm, StorytellingForm, BusinessModelForm } from '../models/workspace.types';
...
export class WorkspaceService {
  ...
  private readonly _workspaceData = signal<WorkspaceData>({
    ...
    personFormData: { personForms: [], currentPersonForm: new PersonForm() },
  ...
  createWorkspace(projectId?: string): Observable<any> {
    const workspaceData = {
      ...
      personFormData: this._workspaceData().personFormData,
    ...
    return this.http.post(this.apiUrl, workspaceData, { headers: this.getAuthHeaders() });
  }
  ...
  saveWorkspaceData(): Observable<any> {
    ...
    if (workspaceId) {
      ...
      return this.http.patch(`${this.apiUrl}/${workspaceId}`, workspaceData, { headers: this.getAuthHeaders() });
    } else {
      // Créer un nouveau workspace
  ...
  getAllWorkspaces(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/user/me`, { headers: this.getAuthHeaders() });
  }
    ...
    this.getWorkspaceById(workspaceId).subscribe({
      next: (workspace) => {
        ...
        this._workspaceData.set({
          ...
          personFormData: workspace.personFormData || { personForms: [], currentPersonForm: new PersonForm() },
